/* ============ app_example18dae141e4b1f33bbf7d044aa308513f =============*/

/* Autogenerated code from Flexx. Code Subject to the BSD-2-clause license. */



flexx.define("app_example18dae141e4b1f33bbf7d044aa308513f", ["pyscript-std.js", "flexx.ui._widget"], function (_py, flexx_ui__widget) {
...
Example.prototype.__handlers__ = ["_Widget__children_changed", "_Widget__container_changed", "_Widget__css_class_changed", "_Widget__icon_changed", "_Widget__make_singleton_container_widgets_work", "_Widget__style_changed", "_Widget__title_changed", "_Widget__update_tabindex", "check_size", "on_event"];
...
Example.prototype.on_event = function () {
    var ev, events, id, stub1_seq, stub2_itr, text;
    ...
    for (stub2_itr = 0; stub2_itr < stub1_seq.length; stub2_itr += 1) {
        ...
        id = _pyfunc_truthy(ev.source.title) || ev.source.text;
        if (_pyfunc_truthy(ev.new_value)) {
            text = _pyfunc_op_add((id + " was "), ev.type);
        } else {
            text = _pyfunc_op_add(((id + " was ") + "un-"), ev.type);
        }
        this.label.text = _pyfunc_op_add((text + "<br />"), this.label.text);
    }
    return null;
};
Example.prototype.on_event.nobind = true;
Example.prototype.on_event._connection_strings = ["tree.items**.checked", "tree.items**.selected", "tree.items**.collapsed"];
