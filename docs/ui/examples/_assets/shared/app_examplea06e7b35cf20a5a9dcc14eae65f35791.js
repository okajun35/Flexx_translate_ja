/* ============ app_examplea06e7b35cf20a5a9dcc14eae65f35791 =============*/

/* Autogenerated code from Flexx. Code Subject to the BSD-2-clause license. */



flexx.define("app_examplea06e7b35cf20a5a9dcc14eae65f35791", ["pyscript-std.js", "flexx.ui._widget"], function (_py, flexx_ui__widget) {
...
Example.prototype.__handlers__ = ["_Widget__children_changed", "_Widget__container_changed", "_Widget__css_class_changed", "_Widget__icon_changed", "_Widget__make_singleton_container_widgets_work", "_Widget__style_changed", "_Widget__title_changed", "_Widget__update_tabindex", "check_size", "cur_tab_changed"];
...
Example.prototype.cur_tab_changed = function () {
    var events, next, prev;
    events = Array.prototype.slice.call(arguments);
    prev = events[0].old_value;
    if ((prev !== null)) {
        prev.title = _pymeth_strip.call(prev.title, " *");
    }
    next = events[events.length -1].new_value;
    if ((next !== null)) {
        next.title = next.title + "*";
    }
    if ((next === this.d)) {
        this.t.current = this.a;
    }
    return null;
};
Example.prototype.cur_tab_changed.nobind = true;
Example.prototype.cur_tab_changed._connection_strings = ["t.current"];
