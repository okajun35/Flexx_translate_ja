...
flexx.define("app_with_bokeh", ["pyscript-std.js", "flexx.ui._widget"], function (_py, flexx_ui__widget) {
...
Example.prototype.__handlers__ = ["_Widget__children_changed", "_Widget__container_changed", "_Widget__css_class_changed", "_Widget__icon_changed", "_Widget__make_singleton_container_widgets_work", "_Widget__style_changed", "_Widget__title_changed", "_Widget__update_tabindex", "_update_sine", "check_size"];
...
Example.prototype._update_sine = function () {
    var amp, ds, events, freq, phase, plot, ren, stub1_, stub2_seq;
    events = Array.prototype.slice.call(arguments);
    stub1_ = [this.amp.value, this.freq.value, this.phase.value];
    amp = stub1_[0];freq = stub1_[1];phase = stub1_[2];
    ds = null;
    plot = this.plot2.plot;
    if (_pyfunc_truthy(plot)) {
        stub2_seq = plot.model.renderers;
        for (ren in stub2_seq) {
            if (!stub2_seq.hasOwnProperty(ren)){ continue; }
            ren = stub2_seq[ren];
            if (_pyfunc_truthy(ren.data_source)) {
                ds = ren.data_source;
                break;
            }
        }
    }
    if (_pyfunc_truthy(ds)) {
        window.ds = ds;
        ds.data.y = (function list_comprehension () {var res = [];var x, iter0, i0;iter0 = ds.data.x;if ((typeof iter0 === "object") && (!Array.isArray(iter0))) {iter0 = Object.keys(iter0);}for (i0=0; i0<iter0.length; i0++) {x = iter0[i0];{res.push(_pyfunc_op_mult(amp, (window.Math.sin(_pyfunc_op_add(_pyfunc_op_mult(x, freq), phase)))));}}return res;}).apply(this);
        ds.trigger("change");
    }
    return null;
};
Example.prototype._update_sine.nobind = true;
Example.prototype._update_sine._connection_strings = ["amp.value", "freq.value", "phase.value"];
